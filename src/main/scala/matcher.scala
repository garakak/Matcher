object matcher {
  ...
  case class Order(client: String, operation: Char, currency: Char, price: Int, count: Int)

  val listOfClients = List(Client("C1", 1000, 10, 5, 15, 0),
                           Client("C2", 2000, 15, 4, 12, 1))

  val listOfOrders = List(Order("C1", 'b', 'A', 7, 12),
                          Order("C2", 's', 'A', 7, 12))

  /**
    * Function, that realises "sell" operation
    *
    * Realised via " <char> case => " because there are just 4 currencies,
  ...
  /**
    ...
    *
    * Realised via "case => _" because there are just 4 currencies,
    * if there were more of them, other method would be chosen
    */
